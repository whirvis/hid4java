...
if [[ "$BRANCH" =~ ^issue.* ]]; then
    ...
    read -r -p "Please enter the project name to confirm the release ($PROJECT_NAME): " userInput
...
if [[ "$BRANCH" =~ ^develop ]]; then
    ...
    # Force user to be sure about releasing
    read -r -p "Please enter the agreed version number (e.g. 1.2.3) to confirm the release: " userInput
    echo -e "\033[32mOK\033[0m - Releasing to '$userInput'"
